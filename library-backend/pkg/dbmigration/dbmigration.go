...
// AutoMigrateUp automatically runs database migration up without CLI args
func AutoMigrateUp(cfg *config.Config) {
	ctx := context.Background()
	...
	if err := goose.RunContext(ctx, "up", db, "migration"); err != nil {
		log.Fatalf("db migrate run: %v", err)
	}

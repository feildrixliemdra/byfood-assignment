 ...
 */
function extractUserMessage(errorData: ApiErrorResponse | null, status: number): string {
  ...
  if (errorInfo.type === 'validation' && errorInfo.validationErrors?.length) {
    ...
    toast.error(title, {
      ...
      style: {
        ...
        '--description-color': '#991b1b', // Force red-800 color
      },
      descriptionClassName: '!text-red-800 dark:!text-red-200 !font-medium !important',
    });
  } else {
    toast.error(title, {
